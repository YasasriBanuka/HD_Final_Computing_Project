  ...
  <div style=" background-image: url('images/img/pimg.jpg');">
    ...
    if (isset($_POST['submit'])) {
      ...
      if (!empty($_POST["submit"])) {
        $loginquery = "SELECT * FROM users WHERE username='$username' && password='$password'"; //selecting matching records - password='".md5($password)."'
        ...
        $row = mysqli_fetch_array($result);
            ...
            if (is_array($row)) {
                ...
                $_SESSION["username"] = $row['username']; // Assuming the 'username' field exists in the database

                // Retrieve the username for the success message
                $username = $row['username'];

                // Use JavaScript to show the success message with the username and redirect
                echo "<script>
                    alert('Login successful! Welcome back $username to Fork\'n Easy!');
